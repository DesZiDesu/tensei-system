...
const ASSET_TOKEN = new URL(import.meta.url).searchParams.get('v') || String(Date.now());
const assetUrl = file => new URL(file + '?v=' + encodeURIComponent(ASSET_TOKEN), import.meta.url).href;
const ChatCore = await import(assetUrl('chat-core.js'));
const { installChatUI } = await import(assetUrl('chat-ui.js'));
let chatUI;
...
const DEFAULT_SETTINGS = Object.freeze({
    renderChat: true, npcAuto: true, chatAvatarSize: 64, dialogueWidth: 100,
...
function getSettings() {
    ...
    settings.chatAvatarSize = number(settings.chatAvatarSize, 64, 32, 128);
    settings.dialogueWidth = number(settings.dialogueWidth, 100, 70, 100);
    return settings;
}
...
function npcProfile(value, fallback = {}) {
    ...
    return {
        ...
        ...Object.fromEntries(['appearance','personality','background','goal','equipment','mood'].map(key => [key, text(value[key], text(fallback[key]), 1000)])),
        chatColor: /^#[0-9a-f]{6}$/i.test(value.chatColor || fallback.chatColor || '') ? value.chatColor || fallback.chatColor : ChatCore.speakerColor(value.name || fallback.name),
        chatAvatarSize: value.chatAvatarSize || fallback.chatAvatarSize ? number(value.chatAvatarSize, number(fallback.chatAvatarSize, 64, 32, 128), 32, 128) : null,
...
function chatContextKey() {
    ...
    return `${ctx.groupId || ctx.characterId || ''}:${ctx.getCurrentChatId?.() || ''}`;
}

function stageNpcDraft(state, profile) {
    const data = ChatCore.profileData(profile);
    if (!ChatCore.usable(data.name)) return;
    state.npcDrafts ||= [];
    const index = state.npcDrafts.findIndex(n => ChatCore.nameKey(n.name) === ChatCore.nameKey(data.name));
    const merged = ChatCore.mergeProfile(index >= 0 ? state.npcDrafts[index] : {}, data);
    if (index >= 0) state.npcDrafts[index] = merged;
    else if (state.npcDrafts.length < 200) state.npcDrafts.push(merged);
}

function isNpcName(name, state = getState()) {
    const key = ChatCore.nameKey(name);
    const ctx = SillyTavern.getContext();
    return ChatCore.usable(name) && ![ctx.name1, state.player.name].filter(Boolean).some(n => ChatCore.nameKey(n) === key)
        && !/^(narrator|user|assistant|unknown|guard|merchant|crowd|ผู้บรรยาย|ผู้เล่น|ฝูงชน|ยาม|พ่อค้า)$/i.test(key);
}

function chatApi() {
    return {
        context: () => SillyTavern.getContext(), contextKey: chatContextKey,
        state: getState, settings: getSettings, hasUserReply, notify,
        portrait: id => SillyTavern.libs?.localforage?.getItem(npcPortraitStorageKey(id)),
        async ingest(parsed) {
            const state = getState();
            // Explicit structured names only: never guess identities from prose.
            const profiles = [...parsed.npcs];
            for (const turn of parsed.turns) {
                if (!profiles.some(p => ChatCore.nameKey(p.name) === ChatCore.nameKey(turn.speaker))) profiles.push({ name: turn.speaker });
            }
            let changed = false;
            for (const raw of profiles) {
                if (!isNpcName(raw.name, state)) continue;
                const key = ChatCore.nameKey(raw.name);
                const index = state.npcs.findIndex(n => ChatCore.nameKey(n.name) === key);
                const previous = state.npcs[index];
                if (previous && !ChatCore.missingProfile(previous).length) continue;
                const draft = state.npcDrafts.find(d => ChatCore.nameKey(d.name) === key);
                const merged = ChatCore.mergeProfile(ChatCore.mergeProfile(previous || {}, draft || {}), ChatCore.profileData(raw));
                if (ChatCore.missingProfile(merged).length) {
                    const before = JSON.stringify(state.npcDrafts);
                    stageNpcDraft(state, merged);
                    changed ||= before !== JSON.stringify(state.npcDrafts);
                    continue;
                }
                if (!previous && state.npcs.length >= 200) continue;
                const next = npcProfile(merged, previous || {});
                if (index < 0) state.npcs.push(next); else state.npcs[index] = next;
                state.npcDrafts = state.npcDrafts.filter(d => ChatCore.nameKey(d.name) !== key);
                changed = true;
            }
            if (changed) await persistState(state, 'chat-npc');
        },
        async saveNpc(raw, image, expectedContext) {
            if (chatContextKey() !== expectedContext) throw new Error('แชทเปลี่ยนแล้ว กรุณาเปิดข้อมูลใหม่');
            const prepared = image ? await resizeImageBlob(image) : null;
            if (chatContextKey() !== expectedContext) throw new Error('แชทเปลี่ยนแล้ว ยังไม่ได้บันทึก');
            const state = getState();
            const duplicate = state.npcs.find(n => ChatCore.nameKey(n.name) === ChatCore.nameKey(raw.name) && n.id !== raw.id);
            if (duplicate) throw new Error('มี NPC ชื่อนี้แล้ว กรุณาแก้ไขรายการเดิม');
            const index = state.npcs.findIndex(n => n.id === raw.id);
            if (index < 0 && state.npcs.length >= 200) throw new Error('จำนวน NPC ถึงขีดจำกัด 200 คนแล้ว');
            const next = npcProfile(raw, state.npcs[index] || {});
            if (!next) throw new Error('กรุณากรอกชื่อ');
            if (prepared || raw.removePortrait) {
                const store = SillyTavern.libs?.localforage;
                if (!store) throw new Error('ไม่มีพื้นที่เก็บภาพใน SillyTavern รุ่นนี้');
                const key = npcPortraitStorageKey(next.id);
                if (prepared) await store.setItem(key, prepared); else await store.removeItem(key);
                if (chatContextKey() !== expectedContext) throw new Error('แชทเปลี่ยนแล้ว ไม่บันทึกลงแชทใหม่');
                next.hasPortrait = Boolean(prepared);
            }
            if (index < 0) state.npcs.push(next); else state.npcs[index] = next;
            state.npcDrafts = state.npcDrafts.filter(d => ChatCore.nameKey(d.name) !== ChatCore.nameKey(raw.name));
            if (ChatCore.missingProfile(raw).length) stageNpcDraft(state, raw);
            if (next.contactId) ensureContactForNpc(state, next);
            await persistState(state, 'npc-management');
        },
        async deleteNpc(raw) {
            const state = getState();
            state.npcs = state.npcs.filter(n => n.id !== raw.id);
            state.npcDrafts = state.npcDrafts.filter(n => ChatCore.nameKey(n.name) !== ChatCore.nameKey(raw.name));
            for (const c of state.contacts) if (raw.id && c.npcId === raw.id) c.npcId = '';
            await persistState(state, 'npc-management');
        },
        async generateProfile(profile) {
            const ctx = SillyTavern.getContext();
            const transcript = ctx.chat.filter(m => !m.is_system).slice(-8).map(m => `${m.is_user ? 'User' : 'Character'}: ${ChatCore.plainChat(m.mes)}`).join('\n').slice(-24000);
            const prompt = `${ChatCore.chatInstructions({ npcDrafts: [profile] })}\nThis request is PROFILE COMPLETION ONLY. Return exactly ONE profile JSON object, no turns, no XML, no commentary. Preserve filled values. Reference conversation:\n${transcript}\nProfile data:\n${JSON.stringify(ChatCore.profileData(profile))}`;
            return parseJson(await ctx.generateQuietPrompt({ quietPrompt: prompt, skipWIAN: true, responseLength: 4000 }));
        },
    };
}
...
function normalize(candidate, base = defaultState()) {
    ...
    result.version = 12;
    result.npcDrafts = (Array.isArray(source.npcDrafts) ? source.npcDrafts : base.npcDrafts || []).slice(0,200).map(ChatCore.profileData).filter(d => ChatCore.usable(d.name));
...
function patchInstructions() {
    ...
    return [
        ...
        'Create every new NPC as a COMPLETE profile in tensei_chat.npcs (all fields required by the Tensei chat protocol). Partial NPC patches are only for already complete existing NPCs when confirmed facts change. Preserve canonical names and ids; do not create duplicate contacts. Complete any unfinished NPC profile in the next normal reply.',
...
function statePrompt(state, { includeState = true, track = true } = {}) {
    ...
    if (getSettings().renderChat || getSettings().npcAuto) lines.push(ChatCore.chatInstructions(state));
...
function updatePrompt(state = getState()) {
    ...
    const enabled = activeChat && (settings.injectState || settings.autoTrack || settings.renderChat || settings.npcAuto);
...
function renderNpcs(panel, state) {
    ...
    panel.innerHTML = `<button type="button" class="tensei-small-button" data-action="npc-management">NPC Management</button>${heading('NPC Codex', `${state.npcs.length} ${tr('NPCs').toLowerCase()}`, 'fa-solid fa-users')}
...
function applyPatchOperation(state, operation) {
    ...
    if (verb === 'upsert' && value && typeof value === 'object') {
        ...
        if (path === 'npcs') {
            const safe = ChatCore.profileData(value);
            const previous = index >= 0 ? collection[index] : null;
            candidate = { ...(previous || {}), ...safe, stats: { ...previous?.stats, ...safe.stats }, id: previous?.id || uid() };
            if (!isNpcName(candidate.name, state) || (!previous && !getSettings().npcAuto)) return false;
            if ((!previous || ChatCore.missingProfile(previous).length) && ChatCore.missingProfile(candidate).length) {
                stageNpcDraft(state, safe); return true;
            }
            candidate.updatedAt = new Date().toISOString();
            state.npcDrafts = (state.npcDrafts || []).filter(d => ChatCore.nameKey(d.name) !== ChatCore.nameKey(candidate.name));
        }
...
function applyStatePatch(current, patch) {
    ...
    next.npcs.forEach(entry => {
        ...
        entry.chatColor = previous?.chatColor || ChatCore.speakerColor(entry.name);
        entry.chatAvatarSize = previous?.chatAvatarSize || null;
    });
...
async function addSettingsDrawer() {
    ...
    const template = await fetch(assetUrl('settings.html'), { cache: 'no-store' });
    if (!template.ok) throw new Error('Could not load Tensei settings');
    container.insertAdjacentHTML('beforeend', await template.text());
    const settings = getSettings();
    bindCheckbox('tensei-render-chat', 'renderChat', settings, () => { updatePrompt(); chatUI?.refresh(); });
    bindCheckbox('tensei-npc-auto', 'npcAuto', settings, updatePrompt);
    bindSettingControl('tensei-chat-avatar-size', 'chatAvatarSize', settings, () => chatUI?.refresh());
    bindSettingControl('tensei-dialogue-width', 'dialogueWidth', settings, () => chatUI?.refresh());
    document.getElementById('tensei-npc-manager-open')?.addEventListener('click', () => chatUI?.open());
...
function bindChatEvents() {
    ...
    eventSource.on(eventTypes.MESSAGE_RECEIVED, async (messageId, generationType) => {
        const key = chatContextKey();
        try {
            await chatUI?.receive(Number(messageId), generationType);
            if (key !== chatContextKey()) return;
            await processAssistantPatch(Number(messageId), generationType);
            chatUI?.renderChat();
        } catch (error) {
            console.error('[Tensei] Could not save NPC data', error);
            notify('error', 'บันทึก NPC ไม่สำเร็จ กรุณาลองใหม่');
        }
    });
    for (const type of ['CHARACTER_MESSAGE_RENDERED','MESSAGE_UPDATED','MESSAGE_DELETED']) {
        if (eventTypes[type]) eventSource.on(eventTypes[type], () => chatUI?.renderChat());
    }
    if (eventTypes.MESSAGE_EDITED) eventSource.on(eventTypes.MESSAGE_EDITED, id => chatUI?.receive(Number(id)));
    if (eventTypes.MESSAGE_SWIPED) eventSource.on(eventTypes.MESSAGE_SWIPED, id => chatUI?.receive(Number(id)));
    if (eventTypes.GENERATION_ENDED) eventSource.on(eventTypes.GENERATION_ENDED, () => chatUI?.renderChat());
}
...
async function initialize() {
    ...
    try {
        getSettings();
        chatUI = installChatUI(chatApi(), ChatCore);
        applyAppearance();
        buildActivityIndicator();
        buildInterface();
        ...
        console.info('[Tensei System] Role-play interface v1.2.0 loaded.');
    } catch (error) {
        initialized = false;
